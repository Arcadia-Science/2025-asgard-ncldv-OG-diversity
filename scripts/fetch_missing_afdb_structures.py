...
Input: a CSV of missing proteins, supplied with --input-csv. It must contain
       'ProteinID', 'UniProtKB_AC', and 'Local_AFDB_File_Found' columns.
Output: Downloads PDB files to the directory given by --output-dir
        (default: data/downloaded_structures/alphafold_structures/) and
        appends to or creates the log given by --log-csv
        (default: data/structure_download_log_v2.csv).

Example:
    python scripts/fetch_missing_afdb_structures.py \\
        --input-csv path/to/afdb_missing_log_with_uniprot_ac.csv
"""
...
# Default output locations (overridable via command-line arguments). The input
# CSV has no default because it is produced by an upstream cross-check step and
# its path varies per run; it must be supplied with --input-csv.
DEFAULT_AFDB_OUTPUT_DIR = DATA_DIR / "downloaded_structures" / "alphafold_structures"
DEFAULT_MASTER_LOG_CSV_FILE = DATA_DIR / "structure_download_log_v2.csv"
...
def main(INPUT_CSV_FILE: Path, AFDB_OUTPUT_DIR: Path, MASTER_LOG_CSV_FILE: Path):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch AlphaFold DB structures for proteins listed as missing from "
            "the local structure log but having an Avg_pLDDT."
        )
    )
    parser.add_argument(
        "--input-csv",
        ...
        required=True,
        help=(
            "Path to the input CSV listing missing proteins (with "
            f"'{PROTEIN_ID_COL}', '{UNIPROT_AC_COL}', and '{LOCAL_FILE_FOUND_COL}' "
            "columns), produced by the upstream cross-check step."
        ),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=DEFAULT_AFDB_OUTPUT_DIR,
        help=f"Directory to save downloaded AlphaFold PDB files (default: {DEFAULT_AFDB_OUTPUT_DIR}).",
    )
    parser.add_argument(
        "--log-csv",
        type=Path,
        default=DEFAULT_MASTER_LOG_CSV_FILE,
        help=f"Path to the cumulative download log CSV (default: {DEFAULT_MASTER_LOG_CSV_FILE}).",
    )
...
if __name__ == "__main__":
    ...
    main(
        ...
        AFDB_OUTPUT_DIR=args.output_dir,
        MASTER_LOG_CSV_FILE=args.log_csv,
    )
